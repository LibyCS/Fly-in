...
class Builder():
    ...
    def validate_hub(self, info: str) -> None:
        """
        ...
        try:
            ...
            raise ValueError("Invalid number of arguments were given"
                             f" for the hub {info}, please format it "
                             "as such:\n<hub type>: <hub name> <x> <y>"
                             " ([metadata])\nHub type must either be "
                             "start_hub, end_hub or hub\nHub name may be"
                             " whatever you choose but it cannot have a "
                             "'-' or a space in it\nx and y coords must be"
                             " ints seperated by spaces\nMetadata is optional"
                             " but must have square brackets")
    ...
    def validate_connect(self, info: str) -> None:
        """
    ...
    def validate_base(self, zone: str, info: str) -> None:
        """
    ...
    @staticmethod
    def build_hub(data: DataDict, zone: Keys, info: str,
                  meta: (None | str) = None) -> None:
        """
        ...
        name, x, y = info.split()
        if (name in data[Keys.START_HUB.value]
           or name in data[Keys.END_HUB.value]
                ...
                if hub1 == compared_hub:
                    if ((hub1, hub2) in
                       ...
                       data[hub_t.value][hub1]["connection"].keys()):
                        raise ValueError("Duplicate connection found"
                                         f"({hub1}, {hub2})")
                    data[hub_t.value][hub1]["connection"][(hub1, hub2)
                                                          ] = capacity
                    ...
                    if ((hub1, hub2) in
                       ...
                       data[hub_t.value][hub2]["connection"].keys()):
                        raise ValueError("Duplicate connection found"
                                         f"({hub1}, {hub2})")
                    data[hub_t.value][hub2]["connection"][(hub1, hub2)
                                                          ] = capacity

    def extraction(self, line: str, data: DataDict, connect_nb: int) -> None:
        """
        ...
        self.validate_base(zone, info)
        if meta is not None and meta.strip() != "":
            if zone == "connection":
                self.validate_meta_connect(meta)
            else:
                self.validate_meta_hub(zone, meta)
        if zone == "nb_drones":
            ...
            connect_nb += 1
            self.build_connections(data, info, meta)
        else:
            if connect_nb != 0:
            ...
            self.build_hub(data, Keys(zone), info, meta)


class Parse():
    def __init__(self) -> None:
        """
        Initialised an empty data dictionary.
        """
        self.data: DataDict = {"nb_drones": 0, "hub": {}, "start_hub": {},
                               "end_hub": {}}

    def parse(self, fname: TextIO) -> DataDict:
        """
        ...
        for line in fname:
            ...
            if not line or line.startswith("#"):
                ...
                continue
            try:
                Builder().extraction(line, self.data, connect_counter)
            except ValueError as message:
                raise ValueError(f"Error on line {line_nb}: {message}")
            line_nb += 1
        Builder.check_isolated_node(self.data)
        Builder.check_start_end(self.data)
        return self.data
